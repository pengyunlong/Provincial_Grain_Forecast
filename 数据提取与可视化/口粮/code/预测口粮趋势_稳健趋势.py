...
def build_forecast(food_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    wide_df["indicator"] = "口粮"
    wide_df["indicator_code"] = "grain_raw"
...
def plot_group(
    ...
    fig.suptitle(
        f"31省口粮消费分省子图：第{group_no}组（按口粮均值降序，第{start}-{end}位；2025-2035为省级稳健趋势预测）",
        fontsize=14.5,
        y=0.995,
    )
...
def main() -> None:
    ...
    forecast_long.to_csv(TABLE_DIR / "省级口粮稳健趋势预测模型分项_2025-2035.csv", index=False, encoding="utf-8-sig")
    forecast_wide.to_csv(TABLE_DIR / "省级口粮稳健趋势预测宽表_2025-2035.csv", index=False, encoding="utf-8-sig")
    forecast_wide[
        ["省份", "year", "indicator", "indicator_code", "forecast_method", "median_robust_trend"]
    ].to_csv(TABLE_DIR / "省级口粮稳健趋势预测_2025-2035.csv", index=False, encoding="utf-8-sig")
    summary.to_csv(TABLE_DIR / "省级口粮稳健趋势预测诊断_2025-2035.csv", index=False, encoding="utf-8-sig")
    ...
    print(TABLE_DIR / "省级口粮稳健趋势预测_2025-2035.csv")
    print(TABLE_DIR / "省级口粮稳健趋势预测模型分项_2025-2035.csv")
    print(TABLE_DIR / "省级口粮稳健趋势预测宽表_2025-2035.csv")
    print(TABLE_DIR / "省级口粮稳健趋势预测诊断_2025-2035.csv")
